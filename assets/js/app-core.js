...
function fmtHours(mins){mins=Number(mins||0); return (mins/60).toFixed(1)+'h';}
function weekMinutes(s){const now=new Date(); const day=now.getDay(); const diff=(day+6)%7; const start=new Date(now); start.setHours(0,0,0,0); start.setDate(start.getDate()-diff); return (s.sessions||[]).filter(x=>Number(x.mins||0)>0 && new Date(x.ts||x.date).getTime()>=start.getTime()).reduce((a,x)=>a+Number(x.mins||0),0);}
...
window.refreshHomeWidgets=function(){
  ...
  const realSessions=(s.sessions||[]).filter(x=>Number(x.mins||0)>0); const xp=Math.round((s.totalMins||0)*2); const level=Math.max(1,Math.floor(xp/240)+1); const levelBase=(level-1)*240; const levelNext=level*240; const levelPct=Math.max(0,Math.min(100,Math.round((xp-levelBase)/(levelNext-levelBase)*100))); const week=weekMinutes(s); const weekGoal=300; const weekPct=Math.max(0,Math.min(100,Math.round(week/weekGoal*100)));
  const pulseLevel=document.getElementById('pulseLevel'),pulseLevelSub=document.getElementById('pulseLevelSub'),pulseLevelFill=document.getElementById('pulseLevelFill'),pulseSessions=document.getElementById('pulseSessions'),pulseWeek=document.getElementById('pulseWeek'),pulseWeekSub=document.getElementById('pulseWeekSub'),pulseWeekFill=document.getElementById('pulseWeekFill'),pulseXp=document.getElementById('pulseXp');
  if(pulseLevel)pulseLevel.textContent='Lvl '+level; if(pulseLevelSub)pulseLevelSub.textContent=Math.max(0,levelNext-xp)+' XP to next'; if(pulseLevelFill)pulseLevelFill.style.width=levelPct+'%'; if(pulseSessions)pulseSessions.textContent=realSessions.length; if(pulseWeek)pulseWeek.textContent=fmtHours(week); if(pulseWeekSub)pulseWeekSub.textContent='goal: '+fmtHours(weekGoal); if(pulseWeekFill)pulseWeekFill.style.width=weekPct+'%'; if(pulseXp)pulseXp.textContent=xp;
